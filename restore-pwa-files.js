...
const manifest = {
  ...
  icons: [
    {
      src: './assets/icon-192.png?v=4',
      sizes: '192x192',
      type: 'image/png',
      purpose: 'any maskable'
    },
    {
      src: './assets/icon.png?v=4',
      sizes: '512x512',
      type: 'image/png',
      purpose: 'any maskable'
    }
...
// Use a more aggressive regex to catch all variations
html = html.replace(/<!-- Unregister any old service workers[\s\S]*?<\/script>\s*/gi, '');
// Also remove if script contains service worker unregistration logic
html = html.replace(/<script>\s*\(function\(\)\s*\{\s*if\s*\(['"]serviceWorker['"]\s*in\s*navigator\)[\s\S]*?<\/script>\s*/gi, '');
